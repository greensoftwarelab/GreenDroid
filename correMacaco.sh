...
#adb shell "echo 0 > sdcard/trepn/GDflag"
...
adb shell pm grant $package android.permission.READ_EXTERNAL_STORAGE
#adb shell pm grant com.greenlab.trepnlib android.permission.READ_EXTERNAL_STORAGE
#adb shell pm grant com.greenlab.trepnlib android.permission.WRITE_EXTERNAL_STORAGE
(adb shell monkey  -s $monkey_seed -p $package -v $monkey_nr_events ) > /dev/null
if [[ $trace == "-TestOriented" ]]; then
